    ...
    try {
        ...
        app.get('/popular-courses', async (req, res) => {
            const popularCourses = await db.collection('enrolledUsersDetails').aggregate([
                {
                    $group: {
                        _id: '$courseId',        // courseId দি
                        enrollCount: { $sum: 1 } // কতবার এনরোল হয়েছ
                    }
                },
                {
                    $sort: { enrollCount: -1 } // বেশি এনরোল হয়েছে
                },
                {
                    $limit: 6                  // সর্বোচ্চ ৬টা popula
                },
                {
                    $lookup: {                 // courses collection থেকে course
                        from: 'courses',
                        localField: '_id',       // enrolledUsersDetail
                        foreignField: '_id',     // courses coll
                        as: 'courseDetails'
                    }
                },
                {
                    $unwind: '$courseDetails'  // array থেকে একেকট
                },
                {
                    $project: {                // যা যা তথ্য দরকার 
                        courseId: '$_id',
                        enrollCount: 1,
