            ...
            const popularCourses = await db.collection('enrolledUsersDetails').aggregate([
                {
                    $group: {
                        _id: '$courseId',        // cour
                        enrollCount: { $sum: 1 } // কতবার এনর
                    }
                },
                {
                    $sort: { enrollCount: -1 } // বেশি এনরো
                },
                {
                    $limit: 6                  // সর্বোচ্চ ৬টা
                },
                {
                    $lookup: {                 // courses collection থেকে
                        from: 'courses',
                        localField: '_id',       // enrolledUser
                        foreignField: '_id',     // cours
                        as: 'courseDetails'
                    }
                },
                {
                    $unwind: '$courseDetails'  // array থেক
                },
                {
                    $project: {                // যা যা তথ্য
                        courseId: '$_id',
